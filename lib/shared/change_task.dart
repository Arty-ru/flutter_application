...
class ChangeTask extends StatefulWidget {
  final String email;
  final int id;
  final Color color;
  const ChangeTask({
  ...
  @override
  State<ChangeTask> createState() => _AddUserState();
}

class _AddUserState extends State<ChangeTask> {
  ...
  Future<void> getAdminInfo(int id) async {
    ...
    if (result.statusCode >= 200 && result.statusCode <= 299) {
      ...
      setState(() {
        ...
        _descriptionController.text = user.email;
        _workerController.text = user.phone ?? "";
      });
    } else {
      final error =
  ...
  Future<void> submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      var result = await http.post(
        Uri.parse(ApiEndpoints.getTaskInfo),
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
          'Authorization': 'Bearer ${await TokenHandler().getToken()}',
        },
        body: json.encode(widget.id),
      );
      if (admin.statusCode >= 200 && admin.statusCode <= 299) {
        final jsonData = json.decode(result.body);
        task = TaskModel.fromJson(jsonData);
      } else {
        final error =
            admin.body.isNotEmpty
                ? jsonDecode(result.body)['message'] ?? "Произошла ошибка."
      ...
      var res = await http.put(
        Uri.parse(ApiEndpoints.updateTask),
        headers: {
          'Content-Type': 'application/json; charset=UTF-8',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppbar(title: "Изменить заявку", color: widget.color),
      body: Column(
